...
import 'package:innova_live_streem/src/view/api/api_end_point.dart';
import 'package:innova_live_streem/src/view/const/data_save_in_local/save_in_share_pref.dart';
import 'package:innova_live_streem/src/view/const/helper/pref_helper.dart';
...
class ApiClient extends GetxService {
  ...
  static const String noInternetMessage = 'Internet connection failed';
  ...
  static Future<Response> postData(String uri, dynamic body,
    ...
    token = await PrefsHelper.getString(ShareprefSave.deviceId);
    user = await PrefsHelper.getString(ShareprefSave.userId);
    session = await PrefsHelper.getString(ShareprefSave.sessionId);
    var mainHeaders  = {
      'x-device': token,
      'x-user': user,
      'x-session': session,
  ...
  static Future<Response> getData(String uri,
    ...
    try {
      debugPrint('====> API Call: $uri\nHeader: ${headers ?? mainHeaders}');
      final baseUri=Uri.parse(ApiConstant.appBaseUrl + uri);
      if(query!=null){
